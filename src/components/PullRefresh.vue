<template>
    <div
            class="clxsd-pull-refresh"
            ref="scrollBox"
            :style="style"
            @touchstart="touchStart"
            @touchmove="touchMove"
            @touchend="touchEnd">
        <div class="clxsd-pull-refreshing-box">
            <div v-if="moveState < 2" >{{ moveState === 0 ? '' : '释放即可刷新...' }}</div>
            <div v-else><i class="weui-loading"/> 加载中...</div>
        </div>
        <div class="clxsd-pull-present-box">
            <slot/>
        </div>
    </div>
</template>
        ...
        methods:{
            ...
            touchEnd (e) {
                ...
                if (this.moveDistance > 50) {
                    ...
                    this.$emit('refresh', () => {
                        //这里就是成功后的回调了，如果该函数被调用，那就以为着加载数据完成，所以状态要回到0，
                        // 当然需要在父组件调用。
                        this.moveState = 0
                    })
                } else {
