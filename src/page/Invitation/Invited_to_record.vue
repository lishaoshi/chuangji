        ...
        <main>
            <ul v-if="list&&list.length">
                <li v-for="(item,index) in list" :key="index">
                    <div>
                        <!-- {{item.certification.updated_at}} -->
                        <!-- <img v-if="item.supplier" :src="item.supplier.img_cover"> -->
                        <img v-if="item.supplier" :src="item.avatar?item.avatar:(item.supplier)?item.supplier.img_cover:''">
                        ...
                        <img v-if="item.supplier&&item.supplier.status==1" class="approve" src="../../images/认证.svg" alt="">
                        <section>
                            <p style="font-size:.3rem;" v-if="item.supplier">{{item.supplier.display_name}}</p>
                            <p :class="{'supplier':item.supplier}" >{{item.real_name}}<span style="margin-left:.16rem"><b v-if="item.supplier">·</b>{{item.phone_desensite}}</span>  <span class="province_name"  v-if="item.supplier">{{item.supplier.province_name}}<b v-if="item.supplier.city_name">·</b>{{item.supplier.city_name}}</span></p>
                        </section>
