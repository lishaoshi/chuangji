            ...
            <ul v-if="list&&list.length">
                <li v-for="(item,index) in list" :key="index">
                    <div>
                        ...
                        <img v-if="item.status==1" class="approve" src="../../images/认证.svg" alt="">
                        <section>
                            <p>{{item.user.supplier}}</p>
                            <p>{{item.user.real_name}}<span style="margin-left:.16rem">{{item.mobile_desensite}}</span></p>
                        </section>
                        <!-- <span>{{item.mobile_desensite}}</span> -->
                        <!-- <span>2019.10.10</span> -->
                        <span>{{item.created_at_int*1000 |formatDate(fmt = "yyyy/MM/dd")}}</span>
                    </div>
                </li>
            </ul>
